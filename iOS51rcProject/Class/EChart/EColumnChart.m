...
- (void)setDataSource:(id<EColumnChartDataSource>)dataSource
{
    if (_dataSource != dataSource)
    {
        ...
        
        {
            ...
            //float highestValueEColumnChart = 12000;//最大的数值
            int heightGap = self.frame.size.height / 6;
            for (int i = 0; i < 7; i++)
            {
                int valueGap = 2000;//每一行间隔多少
                //横线
                UIView *horizontalLine = [[UIView alloc] initWithFrame:CGRectMake(0, heightGap * i + 3, self.frame.size.width, HORIZONTAL_LINE_HEIGHT)];
                ...
                eColumnChartLabel.textColor = [UIColor blackColor];
                eColumnChartLabel.text = [[NSString stringWithFormat:@"%d ", valueGap * (6 - i)] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];
               
                [self addSubview:eColumnChartLabel];
            }
            
            //纵向竖线
            UIView *leftLine = [[UIView alloc] initWithFrame:CGRectMake(0, 3, 0.5, self.frame.size.height)];
            leftLine.backgroundColor = ELightGrey;
...
- (void)setNormalColumnColor:(UIColor *)normalColumnColor
{
    _normalColumnColor = normalColumnColor;
    [self reloadData];
}
...
- (void)reloadData
{
    ...
    totalColumnsRequired = [_dataSource numberOfColumnsPresentedEveryTime:self];
    float highestValueEColumnChart = [_dataSource highestValueEColumnChart:self].value * 1.1;
    highestValueEColumnChart = 12000.0;
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        {
            //＝＝＝＝＝宽度减去了10=＝＝＝＝＝＝＝
            eColumn = [[EColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5), 0, widthOfTheColumnShouldBe-10, self.frame.size.height)];
        ...
        //柱子上方的数字
        UILabel *lbValue = [[UILabel alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5), self.frame.size.height*(1-eColumn.grade) - 15, widthOfTheColumnShouldBe, 20)];
        lbValue.text = [NSString stringWithFormat:@"%d", (int) eColumnDataModel.value] ;
        lbValue.textColor = _normalColumnColor;
        lbValue.font = [UIFont systemFontOfSize:7];
        ...
        {
            eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5) - 5, self.frame.size.height, widthOfTheColumnShouldBe + 5, 20)];
            [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
    ...
    [UIView animateWithDuration:0.8 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^
    {
        ...
        bottomLine.backgroundColor = ELightGrey;
        //bottomLine.layer.cornerRadius = 2.0;
